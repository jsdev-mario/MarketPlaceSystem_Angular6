...
import { SpinnerService, OrderService, SiteInfoService, SocketService, SseService, StatusService, CommonService } from '../../service';
...
@Component({
    selector: 'app-but-today-order',
    templateUrl: './but-today-order.component.html',
    styleUrls: ['./but-today-order.component.css']
})
export class ButTodayOrderComponent implements OnInit, OnDestroy {
    ...
    initData() {
        ...
        this.order_service.getButcherOrdersInfo({ is_today: true }).then(info_data => {
    ...
    downloadPDF() {
        ...
        const doc_title = `Today Orders of ${this.user.shop.shop_name}`;
        const file_name = `today_orders(${this.getTodayDateTime()})`;
